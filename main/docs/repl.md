...
export default defineComponent({
    ...
    methods: {
        run(e) {
            if (!e.shiftKey && this.stdin?.length > 0) {
                ...
                // Snapshot input for the WASM stdin consumer.
    ...
    async mounted() {
        ...
        function readFromStdin(bufferPtr: number, bufferLength: number): number {
            // Called directly by WASM, so consume from the shared stdin buffer.
...
_Please [report](https://github.com/buzz-language/buzz/issues) any bugs or unexpected behavior_
